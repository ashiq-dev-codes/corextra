...
- Fixed grey corners showing on the floating window: its rounded `Material` was inscribed in a same-sized square bounding box, which a rounded shape can't fully reach into at the corners, so whatever painted behind it (the `Overlay` beneath it clips to a hard rectangular edge by default) showed through there. `DraggableFloatingWidget` now takes an optional `borderRadius` and clips its own bounding box to match, defaulting to no rounding (so the circular bubble, which has nothing behind it to leak through, is unaffected)
